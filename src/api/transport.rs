...
// Immutable completed packs are shared data: a collector can publish an inode
// that the network-facing API process must later read. Temporary files remain
// owner-private until this post-verification transition.
...
fn publish_immutable(
    ...
    match fs::hard_link(temporary_path, final_path) {
        ...
        Err(error) if error.kind() == io::ErrorKind::AlreadyExists => {
            if verify_file(metadata, final_path, limits).is_err() {
                return Err(TransportError::ExistingPackInvalid(
    ...
    // Keep `.staging` private even after the immutable final hard link becomes
    // group-readable. A completed publication removes and syncs this alias.
    fs::remove_file(temporary_path).map_err(|source| TransportError::Publish {
        path: temporary_path.to_path_buf(),
        source,
    })?;
