...
pub fn apply_sample(
    ...
    let drive_fresh = state
        ...
        .map_or(true, |watermark| parsed.drive_timestamp_ms > watermark);
    let charge_fresh = state
        .imported_charge_watermark_ms
        .map_or(true, |watermark| parsed.charge_timestamp_ms > watermark);
    let vehicle_fresh = state
        .imported_state_watermark_ms
        .map_or(true, |watermark| parsed.vehicle_timestamp_ms > watermark);
    if !drive_fresh {
    ...
    update_software_update(&mut state, car_id, &parsed, &mut delta)?;
...
fn maybe_close_drive(
    ...
    let offline_timed_out = sample.phase == VehiclePhase::Offline
        && sample.drive_timestamp_ms.saturating_sub(
            open.positions
                .last()
                .map_or(open.start_date_ms, |position| position.date_ms),
        ) >= OFFLINE_DRIVE_TIMEOUT_MS;
...
fn maybe_open_or_extend_charge(
    ...
    if let Some(open) = state.open_charge.as_mut() {
        ...
        delta
            .open_charge_samples
            .push(open.samples.last().cloned().expect("charge sample"));
    }
    Ok(())
}
...
fn maybe_emit_stationary_position(
    ...
    let interval_elapsed = state.last_stationary_position_at_ms.map_or(true, |last| {
...
fn finalize_drive(open: OpenDrive) -> Result<Option<ClosedDrive>, LifecycleError> {
    let (Some(first), Some(last)) = (open.positions.first(), open.positions.last()) else {
        return Ok(None);
    };
    if open.positions.len() < 2 {
        return Ok(None);
    }
    let (Some(first_odometer), Some(last_odometer)) = (first.odometer, last.odometer) else {
        return Ok(None);
    };
    let distance_km = last_odometer - first_odometer;
    if distance_km < 0.01 {
        return Ok(None);
    }
    if last.date_ms < first.date_ms {
        return Err(LifecycleError::InvalidTimeline);
    }
    let duration_min = ((last.date_ms - first.date_ms) as f64 / 60_000.0).round() as i64;
    ...
    let inside_temp_avg = (inside_count > 0).then_some(inside_sum / f64::from(inside_count));
    let power_max = open
        .positions
        .iter()
    ...
    let power_min = open
        ...
        .filter_map(|position| position.power)
        .reduce(f64::min);
    let ideal_ranges = open
        .positions
        .iter()
        .filter(|position| position.ideal_battery_range_km.is_some() && position.odometer.is_some())
        .filter_map(|position| position.ideal_battery_range_km);
    let start_ideal_range_km = ideal_ranges.clone().next();
    let end_ideal_range_km = ideal_ranges.last();
    let (ascent, descent) = elevation_totals(&open.positions);
    let drive = ProjectionDrive {
        id: open.id,
        car_id: open.car_id,
        optimized_at_ms: None,
        start_date_ms: first.date_ms,
        end_date_ms: last.date_ms,
        ...
        start_geofence: None,
        end_geofence: None,
        start_latitude: Some(first.latitude),
        start_longitude: Some(first.longitude),
        end_latitude: Some(last.latitude),
        end_longitude: Some(last.longitude),
        start_soc: first.battery_level,
        end_soc: last.battery_level,
...
fn finalize_charge(
    ...
    let charge = ProjectionCharge {
        ...
        charge_energy_used_kwh: calculate_energy_used_kwh(&open.samples),
...
fn determine_phases(samples: &[ProjectionChargeSample]) -> Option<f64> {
    ...
    let Some(power_phase_ratio) =
        (!ratios.is_empty()).then(|| ratios.iter().copied().sum::<f64>() / ratios.len() as f64)
    else {
        return None;
    };
...
mod tests {
    ...
    fn materializes_a_complete_drive_with_positions() {
        ...
        assert_eq!(step.delta.drives[0].speed_max, Some(25));
        ...
        assert_eq!(
            step.delta.drives[0].start_ideal_range_km,
            Some(338.8 * 1.609_344)
        );
        assert_eq!(
            step.delta.drives[0].end_ideal_range_km,
            Some(334.8 * 1.609_344)
        );
    ...
    fn charge_aggregate_uses_teslamate_delta_and_ordered_grid_energy() {
        ...
        assert!((charge.start_ideal_range_km.unwrap() - 482.8032).abs() < 1e-9);
        assert!((charge.end_ideal_range_km.unwrap() - 523.0368).abs() < 1e-9);
        assert!((charge.start_rated_range_km.unwrap() - 450.61632).abs() < 1e-9);
        assert!((charge.end_rated_range_km.unwrap() - 490.84992).abs() < 1e-9);
    ...
    fn missing_invalid_and_regressed_nested_timestamps_stay_monotonic() {
        ...
        let drive_positions: Vec<_> = step
            ...
            .map(|position| position.date_ms)
            .collect();
        assert_eq!(drive_positions, vec![start + 2_000, start + 3_000]);
        assert!(
            step.delta
                .positions
                .iter()
    ...
    fn position_thermal_flags_survive_sparse_stream_frames_until_drive_close() {
        ...
        for (index, position) in closed.delta.positions.into_iter().enumerate() {
            assert_eq!(position.speed, Some(if index == 0 { 12 } else { 13 }));
            assert_eq!(position.power, (index == 0).then_some(12.5));
            assert_eq!(position.est_battery_range_km, Some(321.8688));
