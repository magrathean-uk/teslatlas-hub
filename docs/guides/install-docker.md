...
Prepare a newly created named volume once:

```sh
./packaging/docker/prepare-volumes.sh
```

The helper changes only `/var/lib/teslatlas-hub` in that named volume. Routine
startup does not recursively change ownership. Bind-mounted data must already
be owned by UID/GID `10001:10001` with the private modes required by Hub.
...
The health probe uses normal certificate validation and never disables TLS
verification.
...
The checked-in packaging script validates the source image and Compose
