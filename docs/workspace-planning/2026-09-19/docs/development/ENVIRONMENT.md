...
2026-09-19 planning note: the retained Debian guest was stopped at final accepted
cleanup. This reconciliation performs no runtime/provisioning or disk remeasurement.
Figures below are dated observations. Recheck them only when the owner starts the
next goal; do not assume they describe current allocated bytes or live process state.
