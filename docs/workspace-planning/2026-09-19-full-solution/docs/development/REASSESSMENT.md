...
**The full working solution is not complete.** The adoption milestone is complete
within its accepted scope. G0–G7 and secondary
...
## Concrete full-solution gap

The 2026-09-19 read-only check of `hub/tools/companions/catalog-current.json`
found `cohorts: []`. The documented ordinary source install/update path therefore
has no admitted compatible cohort. F6 requires exact reachable commit/catalog
admission and ordinary install/update/rollback evidence, not just local-candidate
acceptance. This planning task did not change the catalog or run that installation.
...
The owner clarified that completion must cover all six products and mandatory
F0–F7, including ordinary source/native/container distributions, all supported active
behavior, HA, fresh named-source/passive evidence and final installed end-to-end
acceptance. L1 installed Debian lifecycle is the first slice, not the goal boundary.
D1/F5 needs fresh named input and remains required for full completion.
