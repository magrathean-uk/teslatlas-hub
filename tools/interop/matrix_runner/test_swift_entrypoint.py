...
class SwiftEntrypointTests(unittest.TestCase):
    ...
    def test_wrapper_imports_only_hash_bound_module_and_passes_closed_capability(self):
        ...
        module.write_text(
            ...
            " assert launcher.controller_observations('s') == {7:{'operation':'verify'}}\n"
            " return 0\n",
            encoding="utf-8",
        )
        module.chmod(0o600)
        ...
        capability = swift_entrypoint.SwiftLauncherCapability(
            workers={"swift_macos": lambda binding, callback: {"binding": binding}},
            observations={7: {"operation": "verify"}},
        )
    ...
    def test_production_entrypoint_stays_pending_without_reviewed_registry(self):
        with self.assertRaises(swift_entrypoint.SwiftEntrypointPending):
            swift_entrypoint.run(self.session, swift_entrypoint.SwiftLauncherCapability({}, {}), None)
