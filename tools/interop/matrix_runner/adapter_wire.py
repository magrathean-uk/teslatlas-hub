...
def read_bound_file(binding: Mapping[str, Any], *, label: str, maximum: int, private: bool = True) -> bytes:
    ...
    try:
        ...
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
            raise WireError(f"{label} is not an exclusive regular file")
    ...
    if len(raw) != metadata.st_size or sha256_bytes(raw) != binding["sha256"]:
...
def file_binding(path: Path | str) -> dict[str, str]:
...
def validate_ready(
    ...
    raw = read_bound_file(value["evidence"], label="ready evidence", maximum=MAX_EVIDENCE_BYTES)
...
def write_ack(
    ...
    ready_raw = read_bound_file(ready_binding, label="ready record", maximum=65_536)
...
def load_reviewed_contract(adapter_id: str, registry: Mapping[str, ReviewedContract]) -> LoadedContract:
    entry = registry.get(adapter_id)
    if entry is None:
        raise ContractPending("pending: reviewed adapter contract is unavailable")
    manifest_raw = read_bound_file({"path": entry.manifest_path, "sha256": entry.manifest_sha256}, label="reviewed adapter manifest", maximum=MAX_FRAME_BYTES)
    ...
    if manifest["schema_version"] != 1 or manifest["adapter_id"] != adapter_id or type(manifest["revision"]) is not int or manifest["revision"] <= 0:
        raise WireError("reviewed adapter manifest identity is invalid")
    if manifest["validator"] != {"path": entry.validator_path, "sha256": entry.validator_sha256}:
        raise WireError("reviewed validator binding is mismatched")
    validator_raw = read_bound_file({"path": entry.validator_path, "sha256": entry.validator_sha256}, label="reviewed adapter validator", maximum=MAX_FRAME_BYTES)
    specification = importlib.util.spec_from_file_location("_teslatlas_matrix_contract_" + adapter_id, entry.validator_path)
    if specification is None or specification.loader is None:
        raise WireError("reviewed validator cannot be loaded")
    ...
    if sha256_bytes(validator_raw) != entry.validator_sha256:
        raise WireError("reviewed validator changed during import")
    return LoadedContract(MappingProxyType(manifest), module)
...
def validate_profile_inputs(manifest: Any, members: Any, expected_digest: str) -> None:
    ...
    manifest_raw = read_bound_file(local_manifest, label="profile SHA256SUMS", maximum=1_048_576)
    try:
    ...
    for name, digest in declared.items():
        ...
        read_bound_file(binding, label="profile member", maximum=1_048_576)
    profile = strict_json(read_bound_file(local_bindings["profile.json"], label="profile.json", maximum=1_048_576))
