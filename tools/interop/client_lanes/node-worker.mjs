...
process.on('message',async message=>{
 ...
 try{
  ...
  channel('undici:request:headers').subscribe(({request,response})=>{
   ...
   responses.push({method:request.method,route:new URL(request.path,endpoint).pathname,status:response.statusCode,request_id:id});
  });
  channel('undici:request:error').subscribe(({request,error})=>{if(String(request.origin)===endpoint)failures.push({method:request.method,route:new URL(request.path,endpoint).pathname,error_code:error.code??error.name})});
